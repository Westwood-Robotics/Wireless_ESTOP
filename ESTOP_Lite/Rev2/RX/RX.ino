...
//    write_register(W_REGISTER+SETUP_AW, CONFIG3);   
    ...
    uint8_t CONFIG1 = 0b01001011;  //Set as RX+MASK_RX_DR
    ...
    gpio_put(PIN_SSTOP_OUT, ESTOP_UP); // Inital status of ESTOP is UP(triggered)
...
//  Serial.println(rxbuf[1]); // rxbuf[1] is the data we are looking for.
  ...
  if(local_status & 0b00000010){
    gpio_put(PIN_SSTOP_OUT, ESTOP_UP);    
  }
  else{
    gpio_put(PIN_SSTOP_OUT, ESTOP_DOWN);
  }
//  Serial.println("    ");
//  Serial.println("    ");
}
